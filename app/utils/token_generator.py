...
Tokens are URL-safe and Telegram-compatible (A-Z a-z 0-9 _ -, <= 64 chars).
Always generated with `secrets` (never `random`) and never derived from the
user's telegram_id, so they cannot be reverse-engineered.
"""
...
def generate_public_token(nbytes: int = _DEFAULT_NBYTES) -> str:
    """Return a random URL-safe public token."""
